...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("propcoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("propcoin://"))
    {
        uri.replace(0, 11, "propcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PropCoin.lnk";
}
...
{
    return GetAutostartDir() / "propcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PropCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  propcoin-qt [" + tr("command-line options") + "]                     " + "\n";
